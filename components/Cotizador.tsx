...
import {
  calcularPanoFijoRecto,
  formatearResultadoPanoFijoRecto,
} from "@/calculadoras/pañoFijo/pañoFijoRectoModena";
...
import { ModalArticuloManual } from "@/calculadoras/ModalArticuloManual";
...
export default function Cotizador() {
  const [tipoVentana, setTipoVentana] =
    useState<TipoVentana>("corrediza2hojas");
  ...
  const showToast = (
    ...
    if (type === "success")
      toast.success(message, { position: "top-right", autoClose: 2200 });
    else if (type === "error")
  ...
  const calcularCotizacionAutomatica = () => {
    try {
      ...
      if (tipoVentana === "pañoFijo") {
        ...
        setResultadoTexto(
          formatearResultadoPanoFijoRecto(r, anchoNum, altoNum),
        );
      } else {
        ...
        if (incluirMosquitero && r.ventana) {
          setResultadoTexto(
            formatearResultadoVentanaConMosquitero(r as any, anchoNum, altoNum),
          );
        } else {
          setResultadoTexto(
            formatearResultadoModena2Hojas(r as any, anchoNum, altoNum),
          );
        }
      }
    } catch (err) {
  ...
  const agregarAlPresupuesto = () => {
    try {
      ...
      // =====================
      if (tipoVentana === "pañoFijo") {
        const detalles = calcularPanoFijoRecto(
          anchoNum,
          altoNum,
          vidrioExteriorId,
          vidrioInteriorId,
        ...
        agregarVentana({
          tipo: "pañoFijo",
          tipoNombre: "Paño Fijo Recto contravidrios Recto",
          ancho: anchoNum,
          alto: altoNum,
          medidas: medidasStr,
  ...
  const handleCopy = async () => {
    try {
      ...
      if (successful)
        showToast("Copiado al portapapeles (fallback)", "success");
      ...
      >
        ...
        <div style={{ overflowY: "auto" }} className="card">
          ...
          >
            ...
            <option value="pañoFijo">Paño Fijo Recto Contravidrio Recto</option>
          </select>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            <button
              onClick={calcularCotizacionAutomatica}
          ...
          <input
            ...
            onChange={(e) => handleClienteChange("domicilio", e.target.value)}
          />
          <input
            className="input mb-2"
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => {
                setCliente(CLIENTE_VACIO);
                showToast("Formulario cliente limpiado", "info");
              }}
              className="btn btn-secondary"
              style={{ flex: 1 }}
            >
            ...
            <button
              onClick={() => {
                showToast("Cliente guardado en memoria temporal", "success");
              }}
              className="btn btn-primary"
              style={{ flex: 1 }}
            >
        ...
        <div style={{ overflowY: "auto" }} className="card">
          ...
          >
            ...
            >
              {resultadoTexto || "Aquí aparecerá el resultado de la cotización"}
            </pre>
          </div>
        ...
        <div style={{ overflowY: "auto" }} className="card">
          ...
          >
            ...
            <div className="text-sm">
              N° <strong>{numeroPresupuesto}</strong>
            </div>
          </div>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <button
              onClick={() => {
                /* si exportaste reiniciarContador lo podés llamar acá */
              }}
              className="btn btn-secondary w-full"
            >
